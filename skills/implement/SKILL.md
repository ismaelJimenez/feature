...
Invoke `wingman:git-commit` with event `before_implement` before proceeding.
...
Note: This command assumes a complete task breakdown exists in tasks.md. If tasks are incomplete or missing, suggest running `wingman:tasks` first to regenerate the task list.

10. Invoke `wingman:git-commit` after completion with event name `after_implement`.
