...
**Important**: Each `wingman:checklist` command invocation uses a short, descriptive checklist filename and either creates a new file or appends to an existing one. This allows:
...
Invoke `wingman:git-commit` after completion with event name `after_checklist`.
