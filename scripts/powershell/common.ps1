...
function Find-SpecifyRoot {
    ...
    while ($true) {
        if (Test-Path -LiteralPath (Join-Path $current ".wingman") -PathType Container) {
            return $current
        }
...
function Get-FeaturePathsEnv {
    ...
    $featureJson = Join-Path $repoRoot '.wingman/feature.json'
...
function Resolve-Template {
    ...
    $base = Join-Path $RepoRoot '.wingman/templates'
    ...
    $presetsDir = Join-Path $RepoRoot '.wingman/presets'
    ...
    $extDir = Join-Path $RepoRoot '.wingman/extensions'
    if (Test-Path $extDir) {
