    ...
    foreach ($line in Get-Content $configFile) {
        ...
        if ($inAutoCommit) {
            ...
            if ($inEvent) {
                ...
                if ($line -match '\s+optional:\s*(.+)$') {
                    $val = $matches[1].Trim().ToLower()
                    if ($val -eq 'true') { $optional = $true }
                }
                if ($line -match '\s+message:\s*(.+)$') {
                    $commitMsg = $matches[1].Trim() -replace '^["'']' -replace '["'']$'
                }
                if ($line -match '\s+prompt:\s*(.+)$') {
                    $promptText = $matches[1].Trim() -replace '^["'']' -replace '["'']$'
                }
            }
        }
    }
...
if ($optional) {
    ...
    if (-not $promptText) {
        if ($phase -eq 'before') {
            $promptText = "Commit outstanding changes before ${commandName}?"
        } else {
            $promptText = "Commit changes after ${commandName}?"
        }
    }
    Write-Output "[wingman:prompt] $promptText"
    Write-Output "[wingman:commit-message] $commitMsg"
    exit 0
}
