...
_find_project_root() {
    local dir="$1"
    while [ "$dir" != "/" ]; do
        if [ -d "$dir/.wingman" ] || [ -d "$dir/.git" ]; then
            echo "$dir"
            return 0
        fi
        dir="$(dirname "$dir")"
...
if [ -f "$_config_file" ]; then
    ...
    # Look for auto_commit.<event_name>.enabled, .optional, .message, and .prompt
    ...
    while IFS= read -r _line; do
        ...
        if $_in_auto_commit; then
            ...
            if $_in_event; then
                ...
                if echo "$_line" | grep -Eq '[[:space:]]+optional:'; then
                    _val=$(echo "$_line" | sed 's/^[^:]*:[[:space:]]*//' | tr -d '[:space:]' | tr '[:upper:]' '[:lower:]')
                    [ "$_val" = "true" ] && _optional=true
                fi
                if echo "$_line" | grep -Eq '[[:space:]]+message:'; then
                    _commit_msg=$(echo "$_line" | sed 's/^[^:]*:[[:space:]]*//' | sed 's/^["'\'']//' | sed 's/["'\'']*$//')
                fi
                if echo "$_line" | grep -Eq '[[:space:]]+prompt:'; then
                    _prompt_text=$(echo "$_line" | sed 's/^[^:]*:[[:space:]]*//' | sed 's/^["'\'']//' | sed 's/["'\'']*$//')
                fi
            fi
        fi
    done < "$_config_file"
...
# Handle prompt mode — output signal and exit without committing
if [ "$_optional" = "true" ]; then
    # Generate default prompt text if not configured
    if [ -z "$_prompt_text" ]; then
        if [ "$_phase" = "before" ]; then
            _prompt_text="Commit outstanding changes before ${_command_name}?"
        else
            _prompt_text="Commit changes after ${_command_name}?"
        fi
    fi
    echo "[wingman:prompt] $_prompt_text"
    echo "[wingman:commit-message] $_commit_msg"
    exit 0
fi
